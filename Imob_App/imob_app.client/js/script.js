...
$(function(){
	...
	if($('#contact-form').length || $('#main-contact-form').length)$.include('js/forms.js');
	if($('.kwicks').length)$.include('js/kwicks-1.5.1.pack.js');
	if($('#counter').length)$.include('js/jquery.countdown.js');
	if($('.fixedtip').length||$('.clicktip').length||$('.normaltip').length)$.include('js/jquery.atooltip.pack.js')
// Misc
	$('.list-2 li a').hover(
		function(){$(this).parent().css({backgroundPosition:'0 2px'})},
		function(){$(this).parent().css({backgroundPosition:'0 -50px'})}
	)
// Code
...
$(window).load(function(){
	$('#mp_slider .pagination li').addClass('ie_fix')
})
